...
const getServerURL = () => {
  ...
  if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
    ...
    // 프로덕션 환경: 현재 도메인 사용 (Nginx 프록시를 통해)
    // Nginx가 /api와 /socket.io를 백엔드로 프록시하므로 별도 포트 불필요
...
class SocketManager {
  ...
  connect() {
    ...
    this.socket = io(SERVER_URL, {
      transports: ['websocket', 'polling'], // WebSocket 우선, 폴링 백업
      timeout: 10000, // 타임아웃 단축
      forceNew: true,
      autoConnect: true,
      reconnection: true,
      reconnectionAttempts: 10,
      reconnectionDelay: 1000, // 재연결 지연 단축
      reconnectionDelayMax: 5000, // 최대 재연결 지연 단축
  ...
  async fetchPatients(date = null) {
    try {
      let url = `${SERVER_URL}/api/patients`;
      if (date) {
        url += `?date=${date}`;
      }
      
      // console.log('환자 데이터 요청 URL:', url);
  ...
  async fetchPatientsForDate(date) {
    try {
      // console.log('📅 날짜별 환자 데이터 요청:', date);
      const response = await fetch(`${SERVER_URL}/api/patients/date/${date}`, {
        headers: {
          'ngrok-skip-browser-warning': 'true',
          'Content-Type': 'application/json'
        }
      });
      // console.log('📅 날짜별 환자 데이터 응답 상태:', response.status);
      const data = await response.json();
      // console.log('📅 날짜별 환자 데이터 수신 성공:', data.length, '명');
      return data;
    } catch (error) {
      console.error('📅 날짜별 환자 데이터 조회 실패:', error);
      return [];
    }
  }
  ...
  async updatePatientDate(patientId, newDate) {
    try {
      // console.log('📅 API 호출: 환자 날짜 업데이트', { patientId, newDate });
      
      this.emit('admin_action', {
        type: 'update_patient_date',
        patientId,
        newDate,
        timestamp: new Date()
      });
      
      // console.log('✅ 환자 날짜 업데이트 전송 완료');
      return { success: true };
    } catch (error) {
  ...
  // 날짜별 당직 정보 조회
  async fetchDutyStaff(date) {
    try {
      // console.log('👨‍⚕️ 날짜별 당직 정보 조회:', date);
      const url = date ? `${SERVER_URL}/api/duty?date=${date}` : `${SERVER_URL}/api/duty`;
      const response = await fetch(url, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
      ...
      const data = await response.json();
      // console.log('✅ 날짜별 당직 정보 조회 성공:', data);
      return data;
    } catch (error) {
      console.error('❌ 날짜별 당직 정보 조회 실패:', error);
      return { doctor: '', rn: '', rt: '' }; // 에러 시 기본값 반환
    }
  }

  // 날짜별 당직 정보 업데이트
  async updateDutyStaffForDate(dutyStaff, date) {
    try {
      // console.log('👨‍⚕️ 날짜별 당직 정보 업데이트:', { dutyStaff, date });
      const response = await fetch(`${SERVER_URL}/api/duty/schedule`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': 'true'
        },
        body: JSON.stringify({ dutyStaff, date })
      });
      ...
      const data = await response.json();
      // console.log('✅ 날짜별 당직 정보 업데이트 성공:', data);
      return data;
    } catch (error) {
      console.error('❌ 날짜별 당직 정보 업데이트 실패:', error);
      throw error;
    }
  }
}
